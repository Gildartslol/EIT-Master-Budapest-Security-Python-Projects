...
def try_decryption():
    ...
    for m in ciphers:
        try:
            ...
            print("Integrity chek failed")


def try_signatures():
    key = RSA.importKey(open('my_friend_key.pub').read())
    verifier = PKCS115_SigScheme(key)
    for item in messages:
        msg = item.get('text')
        signature = base64.b64decode(item.get('signature'))
        i = int(signature.hex(), 16)
        h = int.from_bytes(sha512(msg).digest(), byteorder='big')
        signature = pow(i, key.e, key.n)
        print("Signature valid:", h == signature)
